...
from .progress import mark_complete, load_progress, get_completed_count
...
def start_watching(exercises_dir: Path, exercise_info_file: Path) -> None:
    ...
    _run_and_report(current, current_path, exercises)
    
    for _ in watch(str(exercises_dir)):
        progress = load_progress()
        current = get_current_exercise(progress, exercises)
        if current is None:
            print_info("🎉 You've completed all exercises! Amazing work!")
...
def _run_and_report(current, current_path, exercises):
    total = len(exercises)
